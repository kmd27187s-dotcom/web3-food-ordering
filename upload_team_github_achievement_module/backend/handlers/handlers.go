...
func NewServer(cfg config.Config, store repository.Store, chain *blockchain.Client) *Server {
	...
	achievementDemo, err := NewDemoStore(defaultAchievementDemoDataFile)
	if err != nil {
		log.Printf("achievement demo storage fallback to memory: %v", err)
		achievementDemo = newDemoStoreWithoutPersistence()
	}
	return &Server{
		cfg:             cfg,
		members:         members,
...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("GET /api/city", s.handleAchievementDemoCityOverview)
	mux.HandleFunc("GET /api/personal-badges/catalog", s.handleAchievementDemoPersonalBadgeCatalog)
	mux.HandleFunc("GET /api/personal-badges/user", s.handleAchievementDemoPersonalBadges)
	mux.HandleFunc("POST /api/personal-badges/evolve", s.handleAchievementDemoPersonalEvolveBadge)
	mux.HandleFunc("GET /api/store-badges/catalog", s.handleAchievementDemoStoreBadgeCatalog)
	mux.HandleFunc("GET /api/store-badges/store", s.handleAchievementDemoStoreBadges)
	mux.HandleFunc("GET /api/badges/catalog", s.handleAchievementDemoBadgeCatalog)
	mux.HandleFunc("GET /api/badges/user", s.handleAchievementDemoUserBadges)
	mux.HandleFunc("POST /api/badges/evolve", s.handleAchievementDemoEvolveBadge)
	mux.HandleFunc("GET /api/rewards/catalog", s.handleAchievementDemoRewardCatalog)
	mux.HandleFunc("POST /api/rewards/redeem", s.handleAchievementDemoRedeemReward)
