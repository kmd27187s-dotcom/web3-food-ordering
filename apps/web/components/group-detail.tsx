...
import { Copy, Pencil, Trophy } from "lucide-react";
...
export function GroupDetailView({ groupId }: { groupId: number }) {
  ...
  async function handleSaveGroup() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "更新群組資料失敗。");
    } finally {
      setPending(false);
    }
  }
  ...
  async function handleCreateInvite() {
    ...
    try {
      ...
      setMessage(`已建立新的群組邀請碼：${invite.inviteCode}`);
      await refresh();
    } catch (error) {
  ...
  if (loading) return <div className="rounded-[1.5rem] border border-border bg-card p-8">讀取群組資料中...</div>;
  if (!detail) return <div className="rounded-[1.5rem] border border-border bg-card p-8 text-sm text-[hsl(7_65%_42%)]">{message || "目前找不到這個群組。"}</div>;
      ...
      <section className="meal-panel p-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 text-sm leading-7 text-muted-foreground">{detail.group.description || "尚未填寫群組介紹。"}</p>
          </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <Stat label="群組成員" value={`${detail.memberCount} 人`} />
        ...
        {detail.canManage && editing ? (
          <>
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <label className="grid gap-2 text-sm">
                <span className="font-semibold text-foreground">群組介紹</span>
                <input className="meal-field" value={description} onChange={(event) => setDescription(event.target.value)} />
              </label>
            </div>
            <div className="mt-4 flex flex-wrap gap-3">
              <Button onClick={handleSaveGroup} disabled={pending || !name.trim()}>儲存群組資料</Button>
              <Button
                variant="ghost"
                onClick={() => {
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-3 text-sm leading-7 text-muted-foreground">
                這裡保留原本群組會員資訊，另外補上成就排行榜的快速入口。
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button asChild variant="secondary">
                <Link href={`/member/groups/${groupId}/members`}>查看完整會員清單</Link>
              </Button>
          ...
          <div className="mt-6 space-y-3">
            {detail.members.slice(0, 3).map((member) => (
              <div key={member.memberId} className="rounded-[1.2rem] border border-border bg-background/70 p-4">
                ...
                <p className="mt-1 text-sm text-muted-foreground">點數：{member.points}</p>
              </div>
            ))}
            {detail.members.length > 3 ? <p className="text-sm text-muted-foreground">另外還有 {detail.members.length - 3} 位成員，可從完整清單查看。</p> : null}
          </div>
        </div>
        ...
        <div className="meal-panel p-8">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-2xl font-extrabold">群組訂單資訊</h2>
              <p className="mt-3 text-sm leading-7 text-muted-foreground">
                訂單邏輯維持不動，這個測試版本只額外補上排行榜入口。
              </p>
            </div>
            <Button asChild variant="secondary">
              <Link href={`/member/groups/${groupId}/orders`}>查看群組訂單</Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section className="meal-panel p-8">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="text-2xl font-extrabold">邀請碼使用紀錄</h2>
            <p className="mt-3 text-sm leading-7 text-muted-foreground">
              這裡保留群組邀請碼的使用紀錄與時間。
            </p>
          </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {!detail.inviteUsages?.length ? <p className="text-sm text-muted-foreground">目前還沒有邀請碼使用紀錄。</p> : null}
        </div>
      </section>
